...
| 2026-08-02 | Usuário achou a oscilação do ponto de gravação fraca. Causa: ao reduzir o badge pela metade, o alcance do raio também foi reduzido proporcionalmente (7–15px → 3.5–7.5px), e 4px de variação absoluta é pouco perceptível mesmo com o `level` variando o range todo. Alargado para 2.5–12.0px (9.5px de variação, mais que o dobro), com 5px de margem verde sobrando no pico — nunca toca a borda do círculo externo. Sensibilidade à amplitude também subiu de 18x para 24x: fala normal captada pelo microfone embutido é mais baixa que os tons sintéticos usados nos testes anteriores. Mesma mudança nos dois overlays (mac e Windows). |
...
| 2026-08-09 | Usuário reportou o app "abrindo e fechando" ao gravar. Causa: `sd.InputStream()` falhava com `PortAudioError [-9986]` toda vez que o dispositivo de entrada padrão mudava sob o processo (Bluetooth, sleep/wake) — o estado do PortAudio ficava inválido e toda tentativa seguinte falhava até reiniciar o app manualmente. Corrigido em `core/recorder.py`: no primeiro `PortAudioError`, reinicializa o host (`sd._terminate()`/`sd._initialize()`) e tenta abrir de novo uma vez antes de desistir. |
| 2026-08-09 | Ícone do tray no mac trocado para monocromático: só a marca central (sem o círculo verde de fundo), branca, pra combinar com os outros ícones da menu bar (que são silhuetas sobre uma barra translúcida). Novo asset `assets/flow-st8-icon-mono.png` (extraído do `.svg` por distância de cor, sem depender de renderizador SVG externo), adicionado ao spec de empacotamento. Ajustado depois para ~72% do canvas (margem), porque sem padding ficava maior que os ícones vizinhos. |
| 2026-08-09 | "Quit" pelo tray não fechava de verdade — o app reabria sozinho ~1-2s depois. Causa: `sfltool dumpbtm` mostrou o flow-st8 registrado como Login Item persistente (`enabled, allowed, notified`); o macOS relança processos que somem enquanto ainda "habilitados" nesse registro, independente do `launchd`/`KeepAlive` (que estava e continua `False`). Corrigido com `autostart.deactivate_session()` — descarrega o LaunchAgent da sessão atual (`launchctl bootout`) no Quit, sem apagar o plist, então o autostart no próximo login real continua intacto. |
| 2026-08-09 | Investigando o Quit, achei via `lsof` que o singleton lock (`core/singleton.py`) ficava preso mesmo com o processo principal morto: o processo auxiliar `multiprocessing.resource_tracker` (torch/whisper) herdava o file descriptor do lock através de fork antes do exec e continuava segurando-o. Todo launch seguinte batia em "Another instance is already running" mesmo sem nenhum processo real rodando. Corrigido marcando o fd como `FD_CLOEXEC` explicitamente logo após abrir o lock. |
| 2026-08-09 | Três ajustes visuais no overlay do mac. (1) A dica de mãos-livres mostrava o combo cru (`ctrl+alt+o → hands-free`, formato igual em qualquer plataforma) — trocado por `keys.display_words()` (novo em `core/keys.py`), que escreve por extenso ("Control+Option+O") no idioma nativo da tecla. (2) A dica era texto solto sem fundo, ilegível sobre qualquer papel de parede, e aumentar sua altura empurrava a bolinha de gravação pra cima (o painel crescia a partir do canto inferior-direito fixo, e a bolinha era desenhada num offset fixo dentro do canvas, então virava alvo móvel). Redesenhado como card verticalmente centralizado com o "callout" (fundo quase-preto, texto branco, fonte 13pt) — layout mudou de vertical (acima) pra horizontal (à esquerda do badge) depois que a primeira versão estourava o texto pra fora do balão; badge agora sempre no mesmo canto, altura do painel fixa em `_BADGE`, só a largura cresce pra esquerda. (3) O spinner de loading (girando) foi trocado por logo estático, usando o mesmo callout pra "Carregando modelo…". |
| 2026-08-09 | Mesmo redesign do overlay (callout escuro, badge de posição fixa, loading estático) portado pra `backends/windows/overlay.py`, adaptado pra Tk (medição via fonte Pillow/FreeType em vez de `tkfont`, pra não divergir do que é de fato desenhado — o mesmo tipo de estouro que apareceu no mac). **Não verificado numa máquina Windows real** — só sintaxe e a lógica de desenho isolada (Pillow puro, sem Tk/ctypes.windll, que não rodam no mac). Precisa de teste manual no Windows antes de confiar de olhos fechados. |
